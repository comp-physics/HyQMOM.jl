...
| `riemann_solver` | **OPT-IN, default `:hll`.** Interface flux for the high-order path. `:hll` = original two-wave HLL (byte-identical default); `:rusanov` = robust local Lax–Friedrichs fallback. Planned: `:hllc`, `:hllem`, `:kinetic` (the clever, low-diffusion solvers — see `docs/riemann-solver-scope.md`). Unknown values raise `ArgumentError`. |
