...
| `riemann_solver` | **OPT-IN, default `:hll`.** Interface flux for the high-order path. `:hll` = original two-wave HLL (byte-identical default); `:rusanov` = robust local Lax–Friedrichs; `:hllc` = contact-restoring HLLC (implemented + verified genuine, with a built-in realizability fallback to HLL). **Note:** on the high-Mach crossing jets `:hllc` ≈ `:hll` because its star states leave the realizable cone in the collision and fall back — see `docs/riemann-solver-scope.md` §6b; the low-diffusion win needs `:hllem`/`:kinetic` (planned). Demo env: `REPRO_RS=hll|rusanov|hllc`. Unknown values raise `ArgumentError`. |
